...
def success_response(data: Dict[str, Any]) -> Dict[str, Any]:
    """Format successful response for InlineAgent with API schema"""
    return {
        'messageVersion': '1.0',
        'response': {
            'actionGroup': 'TavilySearchProcessor',
            'apiPath': '/search-drug-reviews',  # Must match the path in OpenAPI schema
            'httpMethod': 'POST',  # Must match the method in OpenAPI schema
            'httpStatusCode': 200,
            'responseBody': {
                'application/json': {
...
def error_response(error_message: str) -> Dict[str, Any]:
    """Format error response for InlineAgent with API schema"""
    return {
        'messageVersion': '1.0',
        'response': {
            'actionGroup': 'TavilySearchProcessor',
